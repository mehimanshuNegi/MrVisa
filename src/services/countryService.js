...
export function normalizeCountry(raw) {
  if (!raw) return null;
  return {
    id: raw.id || raw.countryId || raw.code?.toLowerCase(),
    name: raw.name || raw.displayName || raw.country || '',
    displayName: raw.displayName || raw.name || '',
    code: raw.code || '',
    flag: raw.flag || raw.flagEmoji || '🌍',
    flagEmoji: raw.flagEmoji || raw.flag || '🌍',
    flagUrl: raw.flagUrl || '',
...
class CountryService {
  ...
  async createCountry(countryData) {
    const normalized = normalizeCountry({
      ...countryData,
      id: countryData.id || countryData.name?.toLowerCase().replace(/\s+/g, '-') || `country-${Date.now().toString(36)}`
    });

    if (isMockMode()) {
      const all = await this.getAllCountries();
      const updatedList = [normalized, ...all.filter((c) => c.id !== normalized.id)];
